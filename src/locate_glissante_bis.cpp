...
//=========== ON ECRIT SUR LIMAGE FINALE
    ...
    double * val_notn = new double[nTaille]{};
    ...
    while(y+hauteur_fenetre<nH){
        ...
        while(x+largeur_fenetre<nW){
            int nouv_x = x+pas_x;
            double valcad = (ecart_type[ind_xcad + ind_ycad*nbcad_x]<0)? 0 : ecart_type[ind_xcad + ind_ycad*nbcad_x];
            //On ajoute une val
            for(int i = 0; i<largeur_fenetre; i++){
                for (int j = 0; j < hauteur_fenetre; j++){
                    val_notn[x+i+(y+j)*nW] += valcad; 
                    nbaddcad[x+i+(y+j)*nW]++;
                }
            }
            x = nouv_x;
    ...
    for (int i = 0; i < nTaille; i++){
        val_notn[i] /= (double) nbaddcad[i];
        if(etmax<val_notn[i]) etmax = val_notn[i];
    }
    ...
    for (int i = 0; i < nTaille; i++){
        ...
        //else
            //cout << (int)ImgOut[i] << "-";
        ImgOut[i] = ceil((val_notn[i] / (etmax)) * 255);
    }
